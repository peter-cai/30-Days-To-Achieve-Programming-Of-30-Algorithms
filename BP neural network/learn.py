...
class Unit:
    ...
    def calc(self, sample):#完成\sum_{i=1}^n(w_i*x+i)的计算
        self.sample = sample[:]
        # zip返回多个序列每次选第k个数组成元组，list(zip(xyz))是新序列，元组个数是多个序列最小值
...
class Layer:
    def __init__(self, input_length, output_length):
        self.units = [Unit(input_length) for i in range(output_length)]#层包含很多的单元
...
def demo():
    ...
    #    [[1,1], [0]]
    #]
    pat = [[[x],[x*]] for x in [rand(-10,10) for i in range(20)]]
    print(pat)
